...
def _convert_convolution(inexpr, keras_layer, etab):
    ...
    I - Input Channels输入通道数
    H - Filter Height卷积核高度
    W - Filter Width卷积核宽度
    O - Output Channels输出通道数
    ...
    if is_deconv:
        ...
        weight = weight.transpose([3, 2, 0, 1])#transpose用于维度转化(可用于变换张量维度对应关系，可以将不同格式的张量转换到相同格式张量)
    else:
        kernel_h, kernel_w, in_channels, n_filters = weight.shape
...
def from_keras(model, shape=None, layout="NCHW"):
    ...
    params = {k: _nd.array(np.array(v, dtype=np.float32)) for k, v in etab.params.items()}#处理参数
