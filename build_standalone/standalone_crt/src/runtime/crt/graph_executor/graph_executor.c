...
int TVMGraphExecutor_SetupOpExecs(TVMGraphExecutor* executor) {
  ...
  for (nid = 0; nid < executor->nodes_count; nid++) {//遍历json文件中的每一个节点
    const TVMGraphExecutorNode* inode = executor->nodes + nid;
    if (strcmp(inode->op_type, "null")) {//若该节点不是op则跳过
      ...
      for (idx = 0; idx < inode->inputs_count; idx++) {
        ...
        uint32_t eid = TVMGraphExecutor_GetEntryId(executor, entry->node_id, entry->index);//找当前节点的输入的节点
        args[idx] = &(executor->data_entry[eid].dl_tensor);
        args_count++;
      }
      for (idx = 0; idx < inode->param.num_outputs; idx++) {//找当前节点参数的节点
...
int32_t TVMGraphExecutor_CreateTVMOp(TVMGraphExecutor* executor, const TVMOpParam* param,
                                     ...
                                     uint32_t num_inputs, TVMPackedFunc* pf) {//找算子对应函数函数
  int status = 0;
  uint32_t idx;
  TVMOpArgs arg_ptr;
